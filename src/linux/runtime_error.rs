...
impl fmt::Display for RuntimeError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::ResourceNameAlreadyInUse => {
                write!(f, "a resource with the same name already exists")
            }
            Self::ResourceAlignmentLargerThanPage => write!(
                f,
                "tried to move a resource into share memory whose alignment is larger than the page size"
            ),
            Self::ResourceTooLargeForPage => write!(
                f,
                "tried to move a resource into shared memory which is larger than a whole page"
            ),
